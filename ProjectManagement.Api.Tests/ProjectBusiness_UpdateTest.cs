...
public class ProjectBusiness_UpdateTest
{
    ...
    {
        var requestInfoMock = new RequestInfo() { UserId = 2 } ;
        ...
        var projectBusiness = new ProjectBusiness(requestInfoMock, projectRepositoryMock.Object);

        var result = await projectBusiness.UpdateAsync(1, project);

        Assert.True(result is UnauthorizedValidation);
    }
}